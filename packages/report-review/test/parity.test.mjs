...
test('baseline parity: unchanged Host packages and client payload/preview/identity guards', async () => {
  const files = execFileSync('git',['ls-tree','-r','--name-only','c57333b','--','packages'],{cwd:'../..',encoding:'utf8'}).trim().split('\n').filter(p => !p.startsWith('packages/report-review/') || (/\/src\//.test(p) && !p.endsWith('client.jsx')));
  // Includes core, source, renderer, connector, all review Host logic and helper modules.
  assert.equal(execFileSync('git',['diff','c57333b','--',...files],{cwd:'../..',encoding:'utf8'}),'');
  const baseline = await load(execFileSync('git',['show','c57333b:packages/report-review/src/client.jsx'],{encoding:'utf8',maxBuffer:3e6}));

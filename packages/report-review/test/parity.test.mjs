...
test('baseline parity: unchanged Host packages and client payload/preview/identity guards', async () => {
  const files = execFileSync('git',['ls-tree','-r','--name-only','1134379','--','packages'],{cwd:'../..',encoding:'utf8'}).trim().split('\n').filter(p => !p.startsWith('packages/report-review/') || (/\/src\//.test(p) && !p.endsWith('client.jsx')));
  // Includes core, source, renderer, connector, all review Host logic and helper modules.
  assert.equal(execFileSync('git',['diff','1134379','--',...files],{cwd:'../..',encoding:'utf8'}),'');
  const baseline = await load(execFileSync('git',['show','1134379:packages/report-review/src/client.jsx'],{encoding:'utf8',maxBuffer:3e6}));
